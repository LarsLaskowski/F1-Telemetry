...
| `20777` | TCP | service | Replay port the `F1ReplayClient` sends recorded packets to |
...
The `F1ReplayClient` uses the same port number over TCP, so the telemetry port
has to be published (and allowed by the firewall of the host) for both protocols
whenever the client runs on another machine than the service, otherwise the
client cannot deliver a single packet.
...
services:
  f1-telemetry:
    ...
    ports:
      ...
      - "20777:20777/tcp"   # packets from the replay client
