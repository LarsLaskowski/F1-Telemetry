...
/// </summary>
[TestClass]
public class PacketSession2022Tests
{
    #region Fields
    ...
    {
        var isFile = File.Exists(@"SampleData/F1-2022-Session.packet");

        if (isFile)
        {
            _packetAnalyzer = new PacketAnalyzer();

            _packetContent = File.ReadAllBytes(@"SampleData/F1-2022-Session.packet");
        ...
        {
            Assert.IsTrue(isFile, "File F1-2022-Session.packet is missing!");
        }
    }
    ...
    public void PacketSessionCheckSession2022IsSessionObject()
    {
        if (_packetData.PacketHeader != null && _packetContent?.Length >= ConstData.F12022SessionSize + ConstData.F12020HeaderSize)
        {
            ...
            {
                isCorrect = sessionData.PacketData is ISessionData2022;
            }
    ...
    public void PacketSessionAiDifficulty2022ExpectedFifty()
    {
        if (_packetData.PacketHeader != null && _packetContent?.Length >= ConstData.F12022SessionSize + ConstData.F12020HeaderSize)
        {
            ...
            if (session is SessionData sessionData && sessionData.PacketData is ISessionData2022 sessionData2022)
            {
                var isCorrect = sessionData2022.AiDifficulty == 50;

                Assert.IsTrue(isCorrect, "Incorrect ai difficulty!");
            }
            else
            {
                Assert.Fail("Invalid session packet, expected F1 2022!");
            }
        }
        else
        {
    ...
    public void PacketSessionTrack2022ExpectedMonaco()
    {
        if (_packetData.PacketHeader != null && _packetContent?.Length >= ConstData.F12022SessionSize + ConstData.F12020HeaderSize)
        {
            ...
            if (session is SessionData sessionData && sessionData.PacketData is ISessionData2022)
            {
                var isCorrect = sessionData.PacketData?.TrackName.Equals("Monaco");
    ...
    public void PacketSessionTrackLength2022ExpectedValue()
    {
        if (_packetData.PacketHeader != null && _packetContent?.Length >= ConstData.F12022SessionSize + ConstData.F12020HeaderSize)
        {
            ...
            if (session is SessionData sessionData && sessionData.PacketData is ISessionData2022)
            {
                var isCorrect = sessionData.PacketData?.TrackLength == 3323;
