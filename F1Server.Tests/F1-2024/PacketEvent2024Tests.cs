...
/// <summary>
/// Class to test event packets from F1 2024
/// </summary>
[TestClass]
public class PacketEvent2024Tests
{
    #region Fields
    ...
    [ClassInitialize]
    public static void PacketEvent2024Init(TestContext testContext)
    {
        var file1 = File.Exists(@"SampleData/F1-2024-Event-TopSpeed.packet");
        var file2 = File.Exists(@"SampleData/F1-2024-Event-Flashback.packet");
        var file3 = File.Exists(@"SampleData/F1-2024-Event-Penalty.packet");

        if (file1 && file2 && file3)
        {
            _packetAnalyzer = new PacketAnalyzer();

            var fileContent = File.ReadAllBytes(@"SampleData/F1-2024-Event-TopSpeed.packet");

            _packetDataTopSpeed = new ReceivedPacketData();

            _packetDataTopSpeed.SetRawData(fileContent);

            fileContent = File.ReadAllBytes(@"SampleData/F1-2024-Event-Flashback.packet");

            _packetDataFlashback = new ReceivedPacketData();

            _packetDataFlashback.SetRawData(fileContent);

            fileContent = File.ReadAllBytes(@"SampleData/F1-2024-Event-Penalty.packet");
        ...
        {
            Assert.IsTrue(file1, "File F1-2024-Event-TopSpeed.packet is missing!");
            Assert.IsTrue(file2, "File F1-2024-Event-Flashback.packet is missing!");
            Assert.IsTrue(file3, "File F1-2024-Event-Penalty.packet is missing!");
        }
    }
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024IsCorrectPacketType()
    {
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024IsCorrectGameVersion()
    {
        var isCorrect = _packetDataFlashback.PacketHeader?.GameVersion == 2024;
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024CheckEventDataIsEventDataObject()
    {
        if (_packetDataPenalty.PacketHeader != null)
        {
            var isCorrect = false;
            var eventData = _packetAnalyzer.GetEventData(_packetDataPenalty.PacketHeader, File.ReadAllBytes(@"SampleData/F1-2024-Event-Penalty.packet"));

            if (eventData is EventData data && data.PacketData is EventData2024)
            {
                isCorrect = true;
            }
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024IsTopSpeedPacketExpectedTrue()
    {
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024IsTopSpeedExpectedValue()
    {
        if (_packetDataTopSpeed.PacketHeader != null)
        {
            var packetData = _packetAnalyzer.GetEventData(_packetDataTopSpeed.PacketHeader, File.ReadAllBytes(@"SampleData/F1-2024-Event-TopSpeed.packet"));
    ...
    public void PacketEvent2024IsTopSpeedVehicleExpectedZero()
    {
        if (_packetDataTopSpeed.PacketHeader != null)
        {
            ...
            if (packetData is EventData eventData && eventData.PacketData?.EventDetails is IEventDataDetails2024 eventDetails)
            {
                Assert.AreEqual(1, eventDetails.VehicleIndex, "Incorrect vehicle value!");
            }
            else
            {
                Assert.Fail("Invalid packet data, expected F1 2024!");
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024IsFlashbackPacketExpectedTrue()
    {
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024IsFlashbackExpectedValue()
    {
        if (_packetDataFlashback.PacketHeader != null)
        {
            var packetData = _packetAnalyzer.GetEventData(_packetDataFlashback.PacketHeader, File.ReadAllBytes(@"SampleData/F1-2024-Event-Flashback.packet"));
            ...
            if (packetData is EventData eventData && eventData.PacketData?.EventDetails is IEventDataDetails2024 eventDetails)
            {
                Assert.AreEqual(2559U, eventDetails.FlashbackFrame, "Incorrect flashback value!");
            }
            else
            {
                Assert.Fail("Invalid packet data, expected F1 2024!");
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024IsPenaltyPacketExpectedTrue()
    {
        if (_packetDataPenalty.PacketHeader != null)
        {
    ...
    /// </summary>
    [TestMethod]
    public void PacketEvent2024IsPenaltyTypeExpectedValue()
    {
        if (_packetDataPenalty.PacketHeader != null)
        {
            var packetData = _packetAnalyzer.GetEventData(_packetDataPenalty.PacketHeader, File.ReadAllBytes(@"SampleData/F1-2024-Event-Penalty.packet"));
            ...
            if (packetData is EventData eventData && eventData.PacketData?.EventDetails is IEventDataDetails2024 eventDetails)
            {
                Assert.AreEqual(PenaltyType.ThisAndPreviousLapInvalidWithoutReason, eventDetails.PenaltyType, "Incorrect penalty type value!");
            }
            else
            {
                Assert.Fail("Invalid packet data, expected F1 2024!");
    ...
    public void PacketEvent2024IsPenaltyVehicleExpectedZero()
    {
        if (_packetDataPenalty.PacketHeader != null)
        {
            ...
            if (packetData is EventData eventData && eventData.PacketData?.EventDetails is IEventDataDetails2024 eventDetails)
            {
                Assert.AreEqual(13, eventDetails.VehicleIndex, "Incorrect vehicle value!");
            }
            else
            {
                Assert.Fail("Invalid packet data, expected F1 2024!");
